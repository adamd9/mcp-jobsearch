...
import { sendDigest, isSmtpConfigured } from "./mailer.js";
...
export function createServer() {
          ...
          try {
            ...
            if (shouldSendDigest) {
              try {
                ...
                console.error(`Error sending automatic digest email: ${error.message}`);
              }
            }
  ...
  mcpServer.tool(
    ...
    async ({ email }) => {
      ...
      // Non-mock mode: just send digest without scraping
      const matches = await getMatchedJobs(0.7);
